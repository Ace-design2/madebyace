  ...
  return (
    <footer className="relative w-full bg-black text-white pt-24 md:pt-40 pb-12 overflow-hidden selection:bg-red-500/30">
      ...
      <div className="relative z-10 max-w-[1400px] mx-auto px-6 lg:px-12 flex flex-col items-center">
        ...
        <div className="w-full flex flex-col items-center text-center space-y-6 md:space-y-10 mb-24 md:mb-32">
          ...
          <div className="pt-6 md:pt-12">
            <Link 
              ...
              className="group relative inline-flex items-center justify-center gap-3 bg-white text-black font-black uppercase tracking-widest px-10 md:px-14 py-4 md:py-6 rounded-full overflow-hidden shadow-[0_0_30px_rgba(255,255,255,0.1)] transition-all duration-500 hover:shadow-[0_0_50px_rgba(255,26,26,0.3)] hover:-translate-y-2"
            >
              ...
              <span className="relative z-10 text-[10px] md:text-[12px] group-hover:text-white transition-colors duration-300">Start a Project</span>
            </Link>
          </div>
        </div>

        {/* Links & Info Grid */}
        <div className="w-full border-t border-white/5 pt-12 pb-16 flex flex-col md:flex-row justify-between items-start md:items-center gap-10 md:gap-0">
        ...
        {/* Bottom Strip */}
        <div className="w-full flex flex-col-reverse md:flex-row justify-between items-center gap-6 border-t border-white/5 pt-8">
          <p className="text-gray-500 text-[10px] md:text-xs font-medium uppercase tracking-widest text-center md:text-left">
            © {new Date().getFullYear()} Crafted by <span className="text-white font-black">ACE.</span> All rights reserved.
          </p>
          
          <button 
            onClick={scrollToTop}
            className="flex items-center gap-3 group text-gray-500 hover:text-white transition-colors duration-300 cursor-pointer"
          >
            <span className="text-[9px] md:text-[10px] uppercase tracking-[0.2em] font-bold">Back to Top</span>
            <div className="w-10 h-10 md:w-12 md:h-12 rounded-full border border-white/10 bg-white/5 flex items-center justify-center group-hover:bg-red-600 group-hover:border-red-500 group-hover:shadow-[0_0_20px_rgba(255,26,26,0.5)] transition-all duration-500">
              <FiArrowUp className="w-4 h-4 md:w-5 md:h-5 text-gray-400 group-hover:text-white group-hover:-translate-y-1 transition-all duration-300" />
            </div>
          </button>
        </div>
